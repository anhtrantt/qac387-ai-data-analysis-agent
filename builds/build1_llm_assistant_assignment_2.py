...
You are a virtual assistant for basic tasks that only sees the dataset schema (columns + dtypes). 
Do not invent columns that are not in the schema.
The output format is (research questions + variables + analysis + clarifying questions)
...
def profile_to_schema_text(profile: dict) -> str:
    ...
    for col in profile["columns"]:          # 1st blank: "columns"
...
def build_chain(
    ...
    if memory:
        prompt = ChatPromptTemplate.from_messages(
            [
                ("system", SYSTEM_PROMPT),          # 2nd blank: SYSTEM_PROMPT
        ...
        base_chain = prompt | llm | StrOutputParser()   # 3rd blank: llm
...
# TODO (Student): Complete the argparse section below.
    ...
    parser.add_argument(
        "--data",                    # 1st blank: "--data"
        type=str,                    # 2nd blank: str
        required=True,                # 3rd blank: True
        help="Path to CSV file",
    )
    parser.add_argument("--report_dir", type=str, default="reports")  # 4th blank: "reports"
    parser.add_argument("--model", type=str, default="gpt-4o-mini")   # 5th blank: "gpt-4o-mini"
    parser.add_argument("--temperature", type=float, default=0.2)     # 6th blank: 0.2

    parser.add_argument(
        "--quiet_schema",
        action="store_true",          # 7th blank: "store_true"
        help="Do not print schema automatically at startup",
    )
    parser.add_argument(
        "--memory",
        action="store_true",          # 8th blank: "store_true"
        help="Enable conversation memory for this session",
    )
    parser.add_argument(
        "--stream",
        action="store_true",          # 9th blank: "store_true"
    ...
    chain = build_chain(
        ...
        stream=args.stream,
        memory=args.memory,           # 10th blank: memory
    )
